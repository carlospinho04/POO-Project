...
public class Regular extends Cliente {
    public Regular(String id, String nome, long nif, String morada, long telefone, String email, String password) {
        super(id, nome, nif, morada, telefone, email, password);
    }
    /**
     * Funcao que verifica o estatuto do cliente.
     * @return Retorna "Cliente_Regular".
     */
    @Override
    ...
    /**
     * Funcao que tem como objetivo reservar um lugar numa viagem que ainda nao aconteceu.E verificado se o codigo da viagem existe.So sera possivel reservar um lugar na viagem.Se existir lugares vagos o cliente, por ser regular, ira pagar o valor total da viagem.
    ...
    protected Reserva reservarViagem(int cod, List<Viagem> lista_v,List<Viagem> lista_v_r, List<Utilizador> lista_u, Data d_atual) {
        ...
        if(v_aux!=null){
            ...
            if(!flag) {
                ...
                if (lugar != -1) {
                    Reserva r_aux = new Aceite(this.id, v_aux.getCodigo(), lugar + 1, v_aux.getPreco(), d_atual);
    ...
    /**
     * Funcao responsavel pelo cancelamento de uma reserva de um utilizador regular.Primeiro e verificada a existencia da viagem passada como argumento, caso exista verificamos a data na qual o cancelamento esta a ser efetuado, caso a diferenca de datas seja superior a uma semana e a reserva seja da classe Aceite e retornado 50% do valor total da reserva, e o estado da reserva passa de Aceite/Espera para Cancelada, caso contrario nao e retornado nenhum valor do pagamento e passa tambem de Aceite/Espera para Cancelada.
     ...
     * @param lista_u Lista de utilizadores que vai servir para guardar no ficheiro as alteracoes feitas num determinado cliente.
     * @param d_atual Data inserida no inicio do programa.
     * @return Retorna uma reserva da classe Cancelada com o retorno de 50% total do dinheiro caso o cancelamento seja feito ate uma semana antes, ou sem retorno do dinheiro caso o cancelamento seja feito depois de uma semana , e null caso a viagem nao seja encontrada.
     */
    @Override
        ...
        if(v_aux!=null){
            ...
            while(it.hasNext()) {
                ...
                if (r.getCod_viagem()==v_aux.getCodigo() && !r.getTipo().equals("Reserva_Cancelada")) {
                    ...
                    lista_v.add(v_aux);
                    Reserva aux;
                    if (d_atual.diferencaDatas(v_aux.getData())>(7*24*60)) {
                        aux = new Cancelada(this.id, cod, r.getLugar(), r.getPagamento() * 0.5, d_atual);
                    }
                    else {
    ...
    /**
     * Funcao responsavel por ativar uma reserva em espera.Sera procurada a viagem na qual a reserva foi feita, a reserva passa a Aceite e o cliente paga  o valor total da viagem por ser um cliente regular.
